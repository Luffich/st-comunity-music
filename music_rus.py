...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Вот это Луффич постарался, смотри что играет',
                               description='```css\n{0.source.title}\n```'.format(self),
                 ...
                 .set_footer(text=f'| by: kill me, luffich#6666')
...
class Music(commands.Cog):
    ...
    @commands.command(name='govnotypoemq22')
    @commands.has_permissions(manage_guild=True)
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Бля чел.. Сейчас музыка не играет, зачем её пропускать? Можешь включить.')
...
bot = commands.Bot('#', description='Крутой бот от Луффича).')
